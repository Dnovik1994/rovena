...
## Scope
Backend API, frontend Mini App, Traefik proxy, and background workers.
...
- **Transport & headers**: CSP/HSTS/XSS/CTO headers at app + Traefik.
...
5. **Security Misconfiguration** — Headers set at Traefik and FastAPI.
...
- Security headers in `main.py` and Traefik configuration.
...
## Gaps / Follow-ups
- Enable TLS termination in production (Traefik + Let’s Encrypt).
